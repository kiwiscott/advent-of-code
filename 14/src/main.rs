...
fn parse_input(file: &str) -> Vec<Ops> {
    //Took this as a 
    lazy_static! {
        static ref RE: regex::Regex =
            regex::Regex::new(r"^(?P<op>\w+)(\[(?P<addr>\d+)\])? = (?P<value>[\dX]+)$").unwrap();
    }

...
fn main() {
    let v = passports("data.txt").iter().filter(|p| valid_passport(p)).count();
    println!("Problem 1: {:?}", v); 

    let v = passports("data.txt").iter().filter(|p| valid_passport_2(p)).count();
    println!("Problem 2: {:?}", v); 


}

pub fn passports(file_name: &str) -> Vec<String> {
...
fn test_ecl() {
    ...
    */
    ...
    ["amx","blz","zrn","grz","zrn","nzl","2025"].iter().for_each(|i|{
        let s = format!("ecl:{}", i);
        let matches = PASSPORT_RULES_2.matches(&s.to_string());
        assert!(!matches.matched(5) , "expected:{:?}", s);
    });

    ["amb","blu","brn","gry","grn","hzl","oth"].iter().for_each(|i|{
        let s = format!("ecl:{}", i);
        let matches = PASSPORT_RULES_2.matches(&s.to_string());
        assert!(matches.matched(5) , "expected:{:?}", s);
    });
...
fn can_take_string_to_password() {
    ...
    tests.insert(true,"ecl:gry pid:860033327 eyr:2020 hcl:#fffffd byr:1937 iyr:2017 cid:147 hgt:183cm");
    tests.insert(false,"iyr:2013 ecl:amb cid:350 eyr:2023 pid:028048884 hcl:#cfa07d byr:1929");
    tests.insert(true,"hcl:#ae17e1 iyr:2013 eyr:2024 ecl:brn pid:760753108 byr:1931 hgt:179cm");
    tests.insert(false,"hcl:#cfa07d eyr:2025 pid:166559648 iyr:2011 ecl:brn hgt:59in");

    for (valid, data) in &tests {
        assert_eq!(valid,  &valid_passport(data), "expected:{:?} data:{:?}",valid, data);
    }
}
